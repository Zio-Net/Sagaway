...
az acr update --name $ACR_NAME --resource-group "SagawaySharedRG" --subscription "$AZURE_SUBSCRIPTION_ID" --admin-enabled true

# Get admin credentials and use them for Docker login
echo "Getting ACR admin credentials..."
ACR_USERNAME=$(az acr credential show --name $ACR_NAME --resource-group "SagawaySharedRG" --subscription "$AZURE_SUBSCRIPTION_ID" --query username -o tsv)
ACR_PASSWORD=$(az acr credential show --name $ACR_NAME --resource-group "SagawaySharedRG" --subscription "$AZURE_SUBSCRIPTION_ID" --query "passwords[0].value" -o tsv)
