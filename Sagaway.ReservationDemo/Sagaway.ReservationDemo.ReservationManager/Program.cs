	...
	[FromServices] ILogger<Program> logger) =>
{
	...
	try
	{
		...
		{
			logger.LogWarning("No Redis endpoints found.");
			return Results.Problem("No Redis endpoints found.");
		}
		...
		try
		{
			var keys = server.Keys(pattern: "booking-management||Booking_*").ToArray();
			foreach (var key in keys)
			{
				...
				{
					deleted++;
					logger.LogInformation("Deleted: {Key}", key);
				}
			}

			keys = server.Keys(pattern: "reservation-manager||saga-log-*").ToArray();
			foreach (var key in keys)
			{
				bool success = await db.KeyDeleteAsync(key);
				if (success)
			...
			keys = server.Keys(pattern: "inventory-management||Inventory_*").ToArray();
			foreach (var key in keys)
			{
				bool success = await db.KeyDeleteAsync(key);
				if (success)
			...
			bool indexDeleted = await db.KeyDeleteAsync("inventory-management||carclass:index");
			if (indexDeleted)
			{
				logger.LogInformation("Deleted: inventory-management||carclass:index");
				deleted++;
			}

			keys = server.Keys(pattern: "inventory-management||ECON").ToArray();
			foreach (var key in keys)
			{
				bool success = await db.KeyDeleteAsync(key);
				if (success)
			...
			keys = server.Keys(pattern: "inventory-management||STD").ToArray();
			foreach (var key in keys)
			{
				bool success = await db.KeyDeleteAsync(key);
				if (success)
			...
			keys = server.Keys(pattern: "inventory-management||LUX").ToArray();
			foreach (var key in keys)
			{
				bool success = await db.KeyDeleteAsync(key);
				if (success)
			...
			logger.LogInformation("Cleared {Count} keys from Redis without touching Dapr metadata.", deleted);
		}
		catch (Exception ex)
		{
			logger.LogError(ex, "Error occurred while trying to clear keys.");
			return Results.Problem("Error while deleting keys.");
		}

		return Results.Ok($"Deleted {deleted} keys.");
	}
	catch (Exception ex)
	{
		logger.LogError(ex, "Error occurred while connecting to Redis");
		return Results.Problem("Failed to connect to Redis. Please try again later.");
	}
})
